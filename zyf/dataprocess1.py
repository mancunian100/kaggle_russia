...
for att in missing_df['columns_name']:
    X = full[full[att].notnull()].drop([att,'sub_area1'], axis=1)
...
#分块，根据提交的额结果，感觉并没有什么用
full.loc[:30471,'sub_area1']=train['sub_area']
full.loc[30471:,'sub_area1']=test['sub_area'].values
#zelenogradsky
#western
#troitsky
#southern
...
full['sub_block']=np.nan

full.loc[full['sub_area1'].isin(block_list1),'sub_block']='zelenogradsky'
full.loc[full['sub_area1'].isin(block_list2),'sub_block']='western'
full.loc[full['sub_area1'].isin(block_list3),'sub_block']='troitsky'
full.loc[full['sub_area1'].isin(block_list4),'sub_block']='southern'
full.loc[full['sub_area1'].isin(block_list5),'sub_block']='southWestern'
full.loc[full['sub_area1'].isin(block_list6),'sub_block']='southEastern'
full.loc[full['sub_area1'].isin(block_list7),'sub_block']='novomoskovsky'
full.loc[full['sub_area1'].isin(block_list8),'sub_block']='northern'
full.loc[full['sub_area1'].isin(block_list9),'sub_block']='northWester'
full.loc[full['sub_area1'].isin(block_list10),'sub_block']='northEastern'
full.loc[full['sub_area1'].isin(block_list11),'sub_block']='eastern'
full.loc[full['sub_area1'].isin(block_list12),'sub_block']='central'


full=pd.get_dummies(full,columns=['sub_block'])
del full['sub_area1']
full.to_csv('kaggle/russia/full1.csv', index=False)